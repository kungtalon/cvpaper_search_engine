...
def search_paper_on_arxiv(src_dir, des_dir):
    ...
    for excel_name in src_excels:
        ...
        if 'arxiv_link_assured' not in data.columns:
            data['arxiv_link_assured'] = pd.Series([2]*len(data))
        arxiv_link_list = []
...
def parse_content_from_arxiv(src_dir, des_dir, download_dir):
    ...
    for excel_name in src_excels:
        ...
        for col in ['workshop', 'arxiv_link', 'supp_link', 'key_words']:
            data[col] = data[col].apply(lambda x: str(x) if str(x) not in ['nan', '', ' '] else '')
 
        if 'arxiv_link_assured' not in data.columns:
            data['arxiv_link_assured'] = pd.Series([0]*len(data))
        all_tex_raw = []
        all_pdf_raw = []
        all_subsections = []
        for title, pdf_link, arxiv_link, assured in tqdm(data[['title', 'pdf_link', 'arxiv_link', 'arxiv_link_assured']].values):
            tex = ''
            pdf_raw = ''
            if title == 'Exploiting Feature Hierarchies With Convolutional Neural Networks for Cultural Event Recognition':
